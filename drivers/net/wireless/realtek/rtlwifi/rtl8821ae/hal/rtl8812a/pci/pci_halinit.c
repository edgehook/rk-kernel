...
/*James,2020/12/8, modify to support LED function*/
...
static void init_hwled(PADAPTER adapter, u8 enable)
{
	...
	rtw_write8(adapter, 0x4e,  0x28);
	if (enable ==1) {
		rtw_write8(adapter, 0x4e,  0x2a);
		RTW_INFO("Turn on HW LED!\n");
	} else {
		RTW_INFO("Turn off HW LED!\n");
	}
}
